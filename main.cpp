...
int main ()
{
  ...
  **/
  ...
  // init pid_steer parameters	
  PID pid_steer = PID();
  double Kp_steer = 0.1, Kd_steer = 0.01, Ki_steer = 0.01, output_lim_max_steer = 1.2;
  pid_steer.Init(Kp_steer, Ki_steer, Kd_steer, output_lim_max_steer, -output_lim_max_steer); 
  
  // init pid_throttle parameters	  
  PID pid_throttle = PID();
  double Kp_throttle = 0.35, Kd_throttle = 0.01, Ki_throttle = 0.2, output_lim_max_throttle = 1.0;
  pid_throttle.Init(Kp_throttle, Ki_throttle, Kd_throttle, output_lim_max_throttle, -output_lim_max_throttle);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          double current_yaw;
//           double target_x, target_y, target_v, dis2tar;
//           target_x = accumulate( x_points.begin( ) , x_points.end( ) , 0.0 )/(double)x_points.size();
//           target_y = accumulate( y_points.begin( ) , y_points.end( ) , 0.0 )/(double)y_points.size();
//           target_v = accumulate( v_points.begin( ) , v_points.end( ) , 0.0 )/(double)v_points.size();
//           dis2tar = sqrt((target_x-x_position)*(target_x-x_position)+(target_y-y_position)*(target_y-y_position));
//           for (int id = 0; id < x_points.size(); id ++)
//           {
//             double x_temp = x_points[id] - x_position;
//             double y_temp = y_points[id] - y_position;
//             if (dis2tar > sqrt(x_temp*x_temp + y_temp*y_temp))
//             {
//               dis2tar = sqrt(x_temp*x_temp + y_temp*y_temp);
//               target_x = x_points[id];
//               target_y = y_points[id];
//               target_v = v_points[id];
//             }
//           }
          desired_steer = angle_between_points(x_position, y_position, x_point[i], y_point[i]);
          desired_steer = correct_angle(desired_steer);
          current_yaw = correct_angle(yaw);
		  error_steer = (desired_steer - yaw);
          error_steer = correct_angle(error_steer);

//           printf ("error_steer: %f \n", error_steer);
          ...
          **/
          ...
          // print steer control data for debug
          printf ("==========================================%d \n", i);
          printf ("desired steer: %f \n", desired_steer);
          printf ("current steer: %f \n", yaw);
          printf ("steer command: %f \n", steer_output);
          ...
          **/
          ...
          error_throttle = v_point[i] - velocity;
          ...
          **/
          ...
          // print throttle control data for debug
          printf ("========================================== \n");
          printf ("desired velocity: %f \n", v_points[i]);
          printf ("current velocity: %f \n", velocity);
          printf ("throttle command: %f \n", throttle);
          printf ("velocity integration error: %f \n", pid_throttle.ctei);
		  printf ("========================================== \n");
          // Save data
